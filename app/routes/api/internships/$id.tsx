...
export async function action({ params, request }: Route.ActionArgs) {
  ...
  // when a valid value is supplied; this never affects public visibility.
  // The UPDATE below references pipeline_status unconditionally, so make sure
  // the column exists for every PUT.
  ...
  const updateResult = await db.execute(
    sql`
      UPDATE public.internships SET
        ...
        pipeline_status = COALESCE(${pipelineStatusValue}, pipeline_status),
        slug = ${slug},
        updated_at = NOW()
      WHERE id = ${id}
      RETURNING *
